<x-forms::form-group :wrap="$showLabel" :name="$attributes->get('id') ?: $id()" :framework="$framework" :inline="$inline" :required="$required" :floating="$floating">
    <div @class([
        'fileinput',
        'fileinput-wrapper',
        $fileInputClass,
    ...
    >
        ...
        <label class="px-8 py-3 bg-orange-600 hover:bg-orange-700">
            <span class="fileinput-new me-2 font-poppins text-md font-semibold text-white">Select an Image</span>
            <i class="bi bi-arrow-up"></i>

            <input
                {!! $attributes->merge([
                    'class' => 'd-none', // Hide the actual file input
                    'required' => $required,
                    'disabled' => $disabled
                ]) !!}
                type="file"
